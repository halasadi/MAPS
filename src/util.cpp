...
Params::Params(const string &params_file, const long seed_from_command_line) {
  try {
    ...
    eems_options.add_options()
      ...
      ("nBlocks", po::value<int>(&nBlocks)->default_value(1200000), "nBlocks")
...
bool Params::check_input_params( ) const {
  ...
  if (!(dfmin>=nIndiv) || !(dfmax<=nBlocks) || !(dfmin<=dfmax)) {
    cerr << "  Error with the DF parameters:" << endl
	 << "  dfmin = " << dfmin << ", dfmax = " << dfmax << endl;
    error = true;
  }
  if (!(nDemes>0) || !(nIndiv>0) || !(nBlocks>0) || !(nBlocks>=nIndiv)) {
    cerr << "  Error with the EEMS parameters:   (Is nBlocks>=nIndiv?)" << endl
	 << "  nIndiv = " << nIndiv << ", nBlocks = " << nBlocks << ", nDemes = " << nDemes << endl;
    error = true;
  }
...
double poisln(const MatrixXd &Lambda, const MatrixXd &lambda, const MatrixXd &cMatrix){
    ...
    for (int i =0; i < n; i++){
        for (int j = i; j < n; j++){
            ll += lambda(i,j)*log(cMatrix(i,j)*Lambda(i,j))-cMatrix(i,j)*Lambda(i,j);
        }
    }
    return(ll);
}

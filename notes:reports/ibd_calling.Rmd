...
L = 3e6
genomeSize = 2.6e9
ibdM = readIBD("data/const_2D/LOD=1/l_1.3e8_N_1.5e4_nhap_200_d_2_rs_20", 200, L)
myImagePlot(ibdM)

pop1 = ibdM[1:100, 1:100]
pop2 = ibdM[101:200, 101:200]
within_sharing = c(pop1[upper.tri(pop1)], pop2[upper.tri(pop2)])

mean(within_sharing)
genomeSize*calculateIntegral(Q, L, r, 1)

betw_sharing = ibdM[1:100, 101:200]
mean(betw_sharing)
genomeSize*calculateIntegral(Q, L, r, 2)

```

Seems like something wrong here. The empirical mean is bigger than the analytical mean for within sharing and much smaller (0) than the analytical mean for within sharing. Could it be the choice of SNPs i.e. most SNPs arise because there is a mutation in one deme and it's absent in the other. Could this property bias you to call more IBD segments within a population and less between populations? More importantely, is this a concern for real data?

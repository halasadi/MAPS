...
workingDir <- "/Users/halasadi/eems2/test/data/4x5/circle_barrier_highpopsize/"
nchr = 20
lowerCutOff = 2e6
upperCutoff = Inf
nhaploids = ndiploids*2
ibdM = readIBD(paste0(workingDir, "circle_barrier_highpopsize_mt_300gen_1.out.qc.ibd"), nhaploids, lowerCutOff, upperCutoff)

for (i in 2:nchr){
  print(paste0("on chr: ", i))
  ibdM = ibdM + readIBD(paste0(workingDir, "circle_barrier_highpopsize_mt_300gen_", i, ".out.qc.ibd"), nhaploids, lowerCutOff, upperCutoff)
}
write.table(ibdM, file = paste0(workingDir, "eems_2_Inf.sims"), quote=FALSE, sep = " ", row.names = FALSE, col.names=FALSE)
